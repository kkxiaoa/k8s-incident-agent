...
test("repair validation stays read-only across keyboard navigation, mobile and reload", async ({ page, baseURL }) => {
  ...
  const panelGap = () => panel.evaluate((element) => {
    ...
    return evidence.getBoundingClientRect().top - element.getBoundingClientRect().bottom;
  });
  await expect.poll(panelGap).toBeGreaterThanOrEqual(12);
  await expect(panel.getByRole("button", { name: "准备修复提案", exact: true })).toBeEnabled();
  await expect(page.getByRole("button", { name: /批准|回滚|Apply/ })).toHaveCount(0);
  const summary = panel.locator("summary").filter({ hasText: "目标约束与 JSON Patch" });
  await summary.focus();
  ...
  expect(JSON.parse(patchJson!)).toEqual(expect.arrayContaining([expect.objectContaining({ op: "replace" })]));
  await expect(panel.getByRole("button", { name: /复制 JSON|展开 JSON/ })).toHaveCount(0);
  await expect(panel.locator("textarea, [contenteditable]")).toHaveCount(0);
...
test("stale repair remains a failed historical validation", async ({ page }) => {
  ...
  await expect(panel.getByRole("button", { name: "审阅并批准" })).toHaveCount(0);
});
...
for (const failure of [
  ...
  test(`${failure.label} failure shows stopped gates without a proposal`, async ({ page }) => {
    ...
    await expect(panel.getByRole("button", { name: "准备修复提案" })).toHaveCount(0);
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
    await page.reload();
