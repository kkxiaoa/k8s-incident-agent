...
2. The publishing job waits for the `release` Environment. It alone has `contents: write`, `packages: write` and `issues: write` (only for the release PR labels), plus `actions: read` and `pull-requests: read`. After approval, it checks the actual protection configuration, that dispatch's recorded approval, and derives the release PR, version and candidate again; any difference stops publication. A same-run rerun is rejected: retries use a **fresh dispatch and fresh approval**, avoiding reuse of an approval not tied to a run attempt.
