...
The [release PR workflow](../.github/workflows/release-please.yml) runs the pinned `googleapis/release-please-action` on each push to `main` in the owning repository. From the Conventional Commits merged since the last published release it maintains one release PR, on branch `release-please--branches--main`, that sets the next version in `.release-please-manifest.json` and adds a `CHANGELOG.md` entry. It never creates tags or GitHub Releases (`skip-github-release`), never checks out or runs repository code, and never approves or merges. Apart from the approved publishing job, only this job holds `contents`, `pull-requests` and `issues` write permission; labels use the issues API.

- Pull requests are squash-merged with their title as the commit subject; merge commits and rebase merges are disabled. Each pull request therefore adds one changelog entry, and its title must be a Conventional Commit.
